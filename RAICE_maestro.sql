...
--   4. Al terminar verás una tabla de verificación con las 26 tablas creadas
--
...
--
-- VARIABLES DE ENTORNO requeridas en Vercel / servidor:
...
CREATE TABLE IF NOT EXISTS raice_config (
  ...
  correction_window_hour    TEXT        DEFAULT '23:59',  -- hora Colombia, ej: "17:00"
  backup_email              TEXT,
  resend_api_key            TEXT,
  created_at                TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS raice_attendance (
  ...
  activity_note     TEXT,                         -- nota libre para status S
  corrected_by      UUID        REFERENCES raice_users(id) ON DELETE SET NULL,
  corrected_at      TIMESTAMPTZ,
  correction_reason TEXT,                         -- 'omision_docente', 'omision_docente_bulk', etc.
  created_at        TIMESTAMPTZ NOT NULL DEFAULT now(),
...
-- ─────────────────────────────────────────────────────────────────────
-- 09. EXCUSAS / PERMISOS DE INASISTENCIA
...
-- ─────────────────────────────────────────────────────────────────────
-- 10. CATÁLOGO DE FALTAS (Manual de Convivencia)
...
-- ─────────────────────────────────────────────────────────────────────
-- 13. ESCALONES DEL PROCESO TIPO I
...
-- ─────────────────────────────────────────────────────────────────────
-- 14. OBSERVACIONES DEL OBSERVADOR DIGITAL
-- ─────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────
-- 16. CITACIONES A PADRES / ACUDIENTES
-- ─────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────
-- 20. LOGS DE ACTIVIDAD DEL SISTEMA
-- ─────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────
-- 21. ACUDIENTES / PADRES DE FAMILIA (portal público)
-- ─────────────────────────────────────────────────────────────────────
...
-- Seguras sobre instalaciones anteriores (DROP … IF EXISTS + ADD)
-- =====================================================================
...
-- Columnas de auditoría de asistencia (si vienen de versión anterior)
...
-- Casos: columnas de falta del catálogo (versiones anteriores sin ellas)
...
-- Proteger registros históricos: ON DELETE RESTRICT en asistencia y retiros
ALTER TABLE raice_attendance
...
-- Al borrar una excusa (desde la app o desde el panel de Supabase)
-- los registros PE de asistencia correspondientes se eliminan solos.
-- =====================================================================
...
BEGIN
  IF OLD.horas IS NOT NULL AND array_length(OLD.horas, 1) > 0 THEN
    -- Excusa con horas específicas: solo borrar esas horas
  ...
  ELSE
    -- Excusa de día completo: borrar todos los PE de ese día
    DELETE FROM raice_attendance
    WHERE student_id = OLD.student_id
      AND date       = OLD.date
      AND status     = 'PE';
...
-- Reemplaza el año 2026 por el año escolar vigente
...
-- Al ejecutar verás una tabla con las 26 tablas y su número de columnas
-- =====================================================================
SELECT
  table_name                                          AS tabla,
  (SELECT COUNT(*)
