...
export class SchedulesController {
  static async function handleSchedules(req, res, user) {
  ...
  static async function getTeacherSchedule(req, res, user) {
  const sb  = getSupabase();
  const url = new URL(req.url, `http://${req.headers.host}`);
  ...
  static async function handleTeacherAbsences(req, res, user) {
  ...
  static async function handleAbsenceReplacement(req, res, user) {
  requireRole(user, 'superadmin', 'admin');
  const sb = getSupabase();
  ...
  static async function getReplacementSuggestions(req, res, user) {
  requireRole(user, 'superadmin', 'admin');
  const sb  = getSupabase();
  const url = new URL(req.url, `http://${req.headers.host}`);
