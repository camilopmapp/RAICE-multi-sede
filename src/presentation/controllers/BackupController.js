...
export class BackupController {
  static async function handlePurge(req, res, user) {
  ...
  static async function handleBackupExport(req, res, user) {
  requireRole(user, 'superadmin');
  ...
  static async function handleBackupCsv(req, res, user) {
  requireRole(user, 'superadmin');
  if (req.method !== 'GET') return res.status(405).end();
  const sb  = getSupabase();
  ...
  static async function handleBackupEmail(req, res, user) {
  requireRole(user, 'superadmin');
  ...
  static async function handleBackupImport(req, res, user) {
  requireRole(user, 'superadmin');
  if (req.method !== 'POST') return res.status(405).end();
