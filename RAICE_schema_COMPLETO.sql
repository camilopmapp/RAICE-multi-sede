...
CREATE TABLE IF NOT EXISTS raice_cases (
  ...
  actions_taken TEXT,
  notes         TEXT,
  falta_id          UUID,  -- referencia al catálogo (se agrega FK después de crear raice_faltas_catalogo)
  falta_numeral     TEXT,  -- desnormalizado para reportes
  falta_descripcion TEXT,  -- desnormalizado para reportes
  falta_categoria   TEXT,  -- categoría de la falta (academica/convivencia/etc)
  otros_involucrados TEXT, -- otros estudiantes involucrados
...
-- Agregar FK de falta_id ahora que raice_faltas_catalogo ya existe
ALTER TABLE raice_cases ADD CONSTRAINT fk_cases_falta
  FOREIGN KEY (falta_id) REFERENCES raice_faltas_catalogo(id) ON DELETE SET NULL;

-- Prevenir duplicados en el catálogo de faltas
ALTER TABLE raice_faltas_catalogo
  ADD CONSTRAINT uq_falta_tipo_cat_num UNIQUE (tipo, categoria, numeral);
